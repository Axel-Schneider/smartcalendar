        ...
        eventClick: function (info) {
            ...
            if(info.event._def.extendedProps.todo == null) showTodo.classList.add('hidden');
                ...
                info.event._def.extendedProps.todo.forEach(task => {
                    ...
                    li.innerHTML = `
                    <div class="flex items-center px-4 py-2 mt-2 rounded-md hover:bg-gray-100" id="task-${task.id}" task-id="${task.id}" onclick="checkTask()">
                        ...
                        <span id="task-text-${task.id}" class="mx-4 font-medium">
                            ${task.description}
                        </span>
                    </div>
                    `
